...
func (st *SectorBlocks) AddPiece(ctx context.Context, size abi.UnpaddedPieceSize, r io.Reader, pdi sealing.PieceWithDealInfo) (sectorID abi.SectorNumber, err error) {
	sectorID, pieceOffset, err := st.Miner.AllocatePiece(padreader.PaddedSize(uint64(size)))
	if err != nil {
		return 0, err
	}

	err = st.writeRef(pdi.DealInfo.DealID, sectorID, pieceOffset, size)
	if err != nil {
		return 0, err
	}

	return sectorID, st.Miner.SealPiece(ctx, size, r, sectorID, pdi)
}
